...
def transocde_to_clf():
    ...
    for file in cube_files:
        ...
        print(filename)
        # subprocess.Popen([r"./ociomakeclf", input_lut_path, output_clf_path], shell=True)
        cmd = "ociomakeclf " + input_lut_path + file + " " + output_clf_path + filename
        # print(cmd)
        # subprocess.run(cmd, **subprocess_args(True))
        # os.system("ls -la")
        os.system(cmd)
        # ociomakeclf(input_lut_path + file, output_clf_path + filename)
